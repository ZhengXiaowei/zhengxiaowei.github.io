...
如果使用`scrollEnd`的话，滑动翻页则会出发两次`_touch`事件，一次是手指离开屏幕的时候，一次是进行上下页滚动的时候

:::

::: tip 新解决
使用偏移值`distX`来判断方向，**正值为从左往右**，**负值为从右往左**，`0`为**点击事件**
:::
...
::: tip 解决
取消dom的绑定事件，通过方向值为`0`来判断是否是点击事件，每次点击都获取手指的触点坐标，从而进行点击翻页操作。
:::
